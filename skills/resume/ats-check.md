...
2. Get it into a file artifact **once**. Routes 1 and 2 already saved it — use that id, don't
   ...
   `save_file_artifact("/abs/path/resume.pdf", title="<Name> — Resume — <Company> <Role - Req> (PDF)", artifact_id=<the PDF export id, or the DOCX export id for a .docx, from the snapshot header, if any>)`,
